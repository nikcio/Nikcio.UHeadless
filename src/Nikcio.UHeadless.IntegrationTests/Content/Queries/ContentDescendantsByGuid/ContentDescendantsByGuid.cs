...
public class ContentDescendantsByGuidTests : IntegrationTestBase
{
    ...
    public async Task GetGeneralContentDescendantsByGuid_Test(string baseUrl, string route, string? culture)
    {
        ...
        var result = await _setup.UHeadlessClient.GetGeneralContentDescendantsByGuid.ExecuteAsync(routeResult.Data!.ContentByAbsoluteRoute!.Key!.Value, culture);
    ...
    public async Task GetNodeIdContentDescendantsByGuid_Test(string baseUrl, string route, string? culture)
    {
        ...
        var result = await _setup.UHeadlessClient.GetNodeIdContentDescendantsByGuid.ExecuteAsync(routeResult.Data!.ContentByAbsoluteRoute!.Key!.Value, culture);
    ...
    public async Task GetFirstNodesContentDescendantsByGuid_Test(string baseUrl, string route, int firstCount, string? culture)
    {
        ...
        var result = await _setup.UHeadlessClient.GetFirstNodesContentDescendantsByGuid.ExecuteAsync(routeResult.Data!.ContentByAbsoluteRoute!.Key!.Value, firstCount, culture);
    ...
    public async Task GetPropertiesContentDescendantsByGuid_Test(string baseUrl, string route, string? culture)
    {
        ...
        var result = await _setup.UHeadlessClient.GetPropertiesContentDescendantsByGuid.ExecuteAsync(routeResult.Data!.ContentByAbsoluteRoute!.Key!.Value, culture);
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(result.Data!.ContentDescendantsByGuid!.Nodes!.All(node => node!.Properties!.All(property => SharedValidation.IsPropertyValueValid(property!.Value!))));
            Assert.That(result.Data!.ContentDescendantsByGuid!.Nodes!.All(node => node!.Children == null || node.Children.All(child => child!.Properties != null && child.Properties.All(property => SharedValidation.IsPropertyValueValid(property!.Value!)))));
        });
    }
}