...
public class AuthFindMembersByEmailQuery : FindMembersByEmailQuery<BasicMember>
{
    /// <inheritdoc/>
    [Authorize]
    public override IEnumerable<BasicMember?> FindMembersByEmail([Service] IMemberRepository<BasicMember> memberRepository, [GraphQLDescription("The email (may be partial).")] string email, [GraphQLDescription("The page index.")] long pageIndex, [GraphQLDescription("The page size.")] int pageSize, [GraphQLDescription("Determines how to match a string property value.")] StringPropertyMatchType matchType)
    {
